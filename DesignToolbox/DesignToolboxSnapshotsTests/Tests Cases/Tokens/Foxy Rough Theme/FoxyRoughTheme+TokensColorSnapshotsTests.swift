...
/// Tests the UI rendering of each **color token** using reference images for `FoxyRoughTheme`
final class FoxyRoughThemeTokensColorSnapshotsTests: TokensColorSnapshotsTestsTestCase {
    ...
    /// This function tests all color tokens in the `FoxyRoughTheme` with the `light` color schemes.
    ...
    @MainActor func testAllColorsFoxyRoughThemeLight() {
        ...
        if theme.colorsCharts != nil {
            XCTFail("The theme does not have yet color charts, that's unexpected")
        }
        if theme.colorsDecorative != nil {
            XCTFail("The theme does not have yet color decorative, that's unexpected")
        }
    ...
    /// This function tests all color tokens in the `FoxyRoughTheme` with the `dark` color schemes.
    /// It iterates through all sections of `NamedColor`, rendering each illustration in a `UIHostingController`,
    /// and captures a snapshot for each color. The snapshots are saved with names indicating the color, theme, and color scheme.
    @MainActor func testAllColorsFoxyRoughThemeDark() {
