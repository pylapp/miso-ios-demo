...
final class BlueCoatThemeTokensOpacitySnapshotsTests: TokensOpacitySnapshotsTestsTestCase {
    ...
    /// This function tests all opacity tokens in the `BlueCoatTheme` with the `light` color scheme.
    ...
    @MainActor func testAllOpacitiesBlueCoatThemeLight() {
        let interfaceStyle = UIUserInterfaceStyle.light
        testOpacities(for: theme, in: interfaceStyle)
    }

    /// This function tests all opacity tokens in the `BlueCoatTheme` with the `dark` color scheme.
    /// It iterates through all `NamedOpacity` cases, rendering each illustration in a `UIHostingController`
    /// and captures a snapshot. The snapshot is saved with a name indicating the elevation, theme, and color scheme.
    @MainActor func testAllOpacitiesBlueCoatThemeDark() {
