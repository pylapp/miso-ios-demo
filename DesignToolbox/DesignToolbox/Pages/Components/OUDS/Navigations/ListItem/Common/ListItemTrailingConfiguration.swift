...
open class ListItemTrailingConfigurationModel: ComponentConfiguration {
    ...
    @MainActor
    func item(for theme: MISOTheme) -> MISOListItemTrailing? {
        switch option {
        case .none:
            nil
        case .text:
    ...
    @MainActor
    private func tag(for theme: MISOTheme) -> MISOTag {
        let defautlTag = MISOTag(label: tagModel.label,
                                 status: tagModel.status(from: theme),
                                 appearance: tagModel.appearance,
                                 shape: tagModel.shape,
                                 size: tagModel.size)
        let loadingTag = MISOTag(loadingLabel: tagModel.label,
                                 progress: tagModel.progress,
                                 shape: tagModel.shape,
                                 size: tagModel.size)
    ...
    private var textType: MISOListItemTrailing.TextType {
...
struct ListItemTrailingConfiguration: View {
    ...
    var body: some View {
        VStack(spacing: theme.spaces.fixedSmall) {
            ...
            case .text:
                MISOChipPicker(title: "app_components_listItem_trailing_textType_tech".localized(),
