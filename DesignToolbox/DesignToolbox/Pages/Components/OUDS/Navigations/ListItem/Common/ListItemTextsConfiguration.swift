...
open class ListItemTextsConfigurationModel: ComponentConfiguration {
    ...
    func textSlot() -> some View {
        MISOInlineAlert(label: "This is a dumb text slot", status: .info)
    }

    @MainActor
    func bottomSlot() -> some View {
        MISOInlineAlert(label: "This is a dumb bottom slot", status: .info)
    }
...
struct ListItemTextsConfiguration: View {
    ...
    var body: some View {
        MISOChipPicker(title: "app_components_listItem_labelContentType_tech".localized(),
        ...
        VStack {
            if configurationModel.labelContentType == .text {
                ...
                MISOSwitchItem("app_components_listItem_boldLabel_tech", isOn: $configurationModel.hasBoldLabel)
            }

            if configurationModel.itemSize == .default {
                let textModeTitle = "app_components_textMode_tech".localized() + " (Overline)"
                MISOChipPicker(title: textModeTitle,
            ...
            MISOSwitchItem("app_components_listItem_textSlot_tech", isOn: $configurationModel.hasSlot)

            MISOSwitchItem("app_components_listItem_bottomSlot_tech", isOn: $configurationModel.hasBottomSlot)
        }
...
enum ListItemLabelContentType: CaseIterable, CustomStringConvertible {
    ...
    var chipData: MISOChipPickerData<Self> {
        MISOChipPickerData(tag: self, layout: .text(text: description))
    }
